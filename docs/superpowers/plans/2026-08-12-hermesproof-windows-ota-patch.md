...
**Goal:** Make HermesProof's signed Windows OTA updater execute npm-backed candidate gates without `spawn EINVAL`, then publish and install a verified v0.9.2 patch superseding v0.9.1 with the completed CodeRabbit review findings.
...
- Apply Hermes ownership to every modification group and the final release flow: call `hermes_get_state`, claim the task, and lock the exact files before editing; edit only files owned by that worker; run allowlisted verification with `hermes_run_gate`; append redacted evidence; release the exact file locks; then release the task only after its required proof is complete.
...
### Task 2: Promote and attest v0.9.2
...
- Produces: consistent v0.9.2 package metadata, docs, site facts, changelog, SBOM, and truth proof.

- [ ] **Step 1: Set all current release facts to 0.9.2 / v0.9.2**
...
- [ ] **Step 4: Commit v0.9.2 release source and proof**
...
**Interfaces:**
- Consumes: v0.9.2 source commit, private local Ed25519 signing key, both remotes, installed v0.9.0 managed root.
- Produces: matching signed releases, v0.9.2 managed installation, enabled healthy updater task, and evidence ledger entries.
...
Push the release branch to GitLab and GitHub, open fresh v0.9.2 merge requests, merge normally through each platform, and restore any temporarily adjusted GitLab merge policy in a `finally` path.

- [ ] **Step 3: Publish matching v0.9.2 release assets**
...
- [ ] **Step 4: Repair-install v0.9.2 and run a real OTA apply**

Use the signed v0.9.2 installer because v0.9.0 cannot cross its broken npm gate. Then run updater `check` and `apply` against the merged stable candidate; require all production gates, atomic activation, health probes, and configuration preservation to pass.
