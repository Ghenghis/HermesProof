...
| Server | 47 MCP tools across coordination, workspace switching, unlock requests, live status, event long-polling, gates, evidence, events, queue pickup, anonymous orchestration, A2A task exchange, Hermes Agent bridging, diagnostics | [`src/server.mjs`](../src/server.mjs) |
...
HermesProof/
├── src/
│   ├── server.mjs                 # MCP entrypoint (47 MCP tools)
│   └── core/
