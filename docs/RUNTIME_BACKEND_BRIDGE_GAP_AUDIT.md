...
4. Add GitHub project/PR parity tools if GitHub release flow needs the same first-class treatment as GitLab.
5. Consider an optional SQLite/vector mirror only if query volume or semantic recall exceeds what the file-backed state database should handle.
