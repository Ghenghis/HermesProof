...
The release branch is the only publication source. GitLab and GitHub must contain the same verified source commit, annotated tag, notes, and signed artifacts. A folder name, modification date, generated proof file, or uncommitted worktree is never evidence that a checkout is current.
...
|---|---|---|
| isolated `hermesproof-release` worktree | contains both remote histories and the stable release changes | source of truth for `v0.9.0` |
| `G:\Github\HermesProof` | clean `v0.9.0-rc.1` checkout; 21 commits behind the audited worktree | refresh after publication; do not publish as-is |
...
1. Fetch both remotes and check out `release/hp-mha-serena-shippable` in a clean, isolated worktree.
2. Verify that both remote default branches are ancestors of the release branch; reconcile with normal merge commits, never a destructive reset.
...
5. Build with the external Ed25519 key, publish identical assets to both providers, download both copies, and verify them independently before declaring the release current.
