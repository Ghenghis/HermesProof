...
This matrix maps the current v0.9.2 README claims to executable evidence. Generated numbers come from config/release-facts.json and scripts/generate-release-docs.mjs.
...
No production-ready claim is accepted solely from documentation. The final release must include a clean-tree local suite, both live MCP probes, truth-gate evidence, Windows install/repair/rollback smoke, checksums, and GitLab asset re-download verification. A successful project-owned runner pipeline is additionally required when a project-owned runner is available; with zero usable GitLab compute allowance, the signed local proof archive is the release gate and no shared-runner job is started.
