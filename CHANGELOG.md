...
- **HP-MHA v4-STABLE** — closed the v3 trace-index and stdio gaps, promoted holdout isolation to a required truth gate, and introduced three seven-layer onboarding examples for OpenHands, Aider, and Goose. The original examples were later replaced by verified installed-reference cards, and v0.9.2 separates card provenance validation from the exact Kilo benchmark attribution. The 121-tool core server and the 11-step real stdio/Merkle smoke remain release-gated.
