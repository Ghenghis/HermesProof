...
- Agent workflow layer: `hermes_update_presence`, `hermes_list_presence`, `hermes_find_agents`, `hermes_request_assistance`, `hermes_send_message`, `hermes_get_inbox`, `hermes_wait_for_inbox`, `hermes_ack_message`, `hermes_wait_for_unlock`, and `hermes_complete_work` add skills-aware routing, durable inboxes, assistance requests, unlock waiting, stale-owner recovery routing, and one-call completion/release.
- Redacted backend/GitLab tools: `hermes_backend_status`, `hermes_gitlab_status`, `hermes_gitlab_ensure_project`, `hermes_gitlab_list_merge_requests`, and `hermes_gitlab_create_merge_request` make GitLab project/MR work first-class without returning token values or private env-file paths.
