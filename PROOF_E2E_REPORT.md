...
- **Run id**: `truth_2026-05-03T17-54-38-824Z`
- **Timestamp (UTC)**: 2026-05-03T17:54:38.824Z
- **Duration**: 4.30s
- **Hermes3D workspace**: `/home/runner/work/HermesProof/HermesProof`
- **Node**: v20.20.2 on linux
- **Result**: ✅ ALL REQUIRED GATES PASS

Pass / Fail / Warn / Skip: **24 / 0 / 2 / 4**

## Gate results

| Gate | Level | Result | Duration | Detail |
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 20 ms | 57 files hashed |
| `deps.parity` | required | ✅ pass | 2 ms | all 3 deps installed |
| `tests.unit` | required | ✅ pass | 1979 ms | pass=124, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 255 ms | 42 tools |
...
| `trigger.doctor_passes` | required | ✅ pass | 79 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 183 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 67 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 383 ms | 14/14 checks; 15 ledger, 11 events |
...
| `secret.scan` | required | ✅ pass | 44 ms | fallback: 0 finding(s) |
...
| `licenses.scan` | required | ✅ pass | 924 ms | 93 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 267 ms | 3/3 direct deps within 12mo |
