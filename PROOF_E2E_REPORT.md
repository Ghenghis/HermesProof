...
- **Run id**: `truth_2026-07-02T12-53-10-510Z`
- **Timestamp (UTC)**: 2026-07-02T12:53:10.510Z
- **Duration**: 45.13s
- **Hermes3D workspace**: `G:\Github\Hermes3D`
- **Node**: v26.2.0 on win32
- **Result**: ✅ ALL REQUIRED GATES PASS

Pass / Fail / Warn / Skip: **29 / 0 / 1 / 5**

## Gate results

| Gate | Level | Result | Duration | Detail |
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 31 ms | 75 files hashed |
| `deps.parity` | required | ✅ pass | 3 ms | all 3 deps installed |
| `tests.unit` | required | ✅ pass | 31006 ms | pass=301, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 441 ms | 55 tools |
| `doctor.hermes3d` | skipped | ✅ pass | 0 ms | skipped |
| `events.directory_present` | required | ✅ pass | 31 ms | outbox/handled/failed present |
| `tasks.directory_present` | required | ✅ pass | 27 ms | pending/claimed/blocked/done present |
| `trigger.doctor_passes` | required | ✅ pass | 300 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 482 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 577 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 823 ms | 14/14 checks; 15 ledger, 11 events |
...
| `security.mcp_scan_pass` | required | ✅ pass | 28 ms | 0 suspicious patterns across 16 signatures |
...
| `lmstudio.health` | warn | ⚠️ warn | 23 ms | LM Studio offline: ECONNREFUSED |
| `ollama.health` | warn | ✅ pass | 9 ms | Ollama reachable (200) |
| `secret.scan` | required | ✅ pass | 120 ms | fallback: 0 finding(s) |
...
| `licenses.scan` | required | ✅ pass | 7970 ms | 93 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 607 ms | 3/3 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 5 ms | 4 workflow(s), 17 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 2382 ms | 0 critical/serious violations across 22 passing rule(s) (1 non-blocking warning(s)) |
...
| `docs.reflects_changes` | warn | ✅ pass | 121 ms | no version bump or ADR change in range; gate is inert |
| `release.checksums_present` | warn | ✅ pass | 0 ms | no release artifacts in dist,release; gate dormant |
| `quality.coderabbit_reviewed` | skipped | ✅ pass | 51 ms | no PR context (owner/repo/pr); gate inert |
