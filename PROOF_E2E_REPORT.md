...
- **Run id**: `truth_2026-05-09T12-22-19-713Z`
- **Timestamp (UTC)**: 2026-05-09T12:22:19.713Z
- **Duration**: 39.34s
...
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 16 ms | 73 files hashed |
| `deps.parity` | required | ✅ pass | 2 ms | all 3 deps installed |
| `tests.unit` | required | ✅ pass | 31482 ms | pass=289, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 238 ms | 42 tools |
...
| `trigger.doctor_passes` | required | ✅ pass | 78 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 174 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 67 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 387 ms | 14/14 checks; 15 ledger, 11 events |
...
| `licenses.scan` | required | ✅ pass | 881 ms | 93 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 186 ms | 3/3 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 5 ms | 4 workflow(s), 17 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 1552 ms | 0 critical/serious violations across 22 passing rule(s) (1 non-blocking warning(s)) |
| `perf.budgets_pass` | required | ✅ pass | 4099 ms | hermes_doctor_cold_start=0.5ms<300ms? Y; lock_acquire=7.3ms<50ms? Y; heartbeat=2.4ms<20ms? Y |
