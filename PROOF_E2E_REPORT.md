...
- **Run id**: `truth_2026-08-12T20-01-15-720Z`
- **Timestamp (UTC)**: 2026-08-12T20:01:15.720Z
- **Duration**: 49.03s
...
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 55 ms | 188 files hashed |
| `deps.parity` | required | ✅ pass | 3 ms | all 7 deps installed |
| `tests.unit` | required | ✅ pass | 31252 ms | pass=497, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 444 ms | 121 tools |
| `doctor.hermes3d` | required | ✅ pass | 5 ms | ok=true, 0 finding(s) |
| `events.directory_present` | required | ✅ pass | 22 ms | outbox/handled/failed present |
...
| `queue.doctor_passes` | required | ✅ pass | 502 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 418 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 842 ms | 14/14 checks; 15 ledger, 11 events |
...
| `clients.claude_code_live` | required | ✅ pass | 5077 ms | 2/2 connected |
| `server.tool_description_hygiene` | required | ✅ pass | 9 ms | 0 suspicious patterns |
| `security.mcp_scan_pass` | required | ✅ pass | 173 ms | 0 suspicious patterns across 16 signatures |
| `evidence.hash_chain_valid` | required | ✅ pass | 38 ms | positive=true, negative_detected_at_idx_1=true |
...
| `backend.api_config_presence` | warn | ✅ pass | 167 ms | backend API inventory: envs=8, gh=false, glab=false |
| `gitlab.auth_probe` | warn | ✅ pass | 305 ms | GitLab authenticated via gitlab_env_file:GITLAB_TOKEN |
| `secret.scan` | required | ✅ pass | 186 ms | fallback: 0 finding(s) |
| `secrets.rotation_evidence_present` | warn | ✅ pass | 1 ms | env mtime 2026-08-08T19:54:02.512Z (age 4.0d <= max 90d) |
| `sbom.cyclonedx_generated` | required | ✅ pass | 47 ms | 96 components @ C:/Users/Admin/Documents/Codex/2026-08-08/g-github-hermes3d-mcp-lock-orchestrator-2/work/hermesproof-release/PROOF/sbom.json |
| `licenses.scan` | required | ✅ pass | 1624 ms | 99 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 1038 ms | 7/7 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 1 ms | 0 workflow(s), 0 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 1174 ms | 0 critical/serious violations across 21 passing rule(s) (1 non-blocking warning(s)) |
...
| `docs.reflects_changes` | warn | ✅ pass | 138 ms | no version bump or ADR change in range; gate is inert |
