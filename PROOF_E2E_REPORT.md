...
- **Run id**: `truth_2026-05-03T20-14-25-376Z`
- **Timestamp (UTC)**: 2026-05-03T20:14:25.376Z
- **Duration**: 9.03s
...
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 15 ms | 73 files hashed |
| `deps.parity` | required | ✅ pass | 2 ms | all 3 deps installed |
| `tests.unit` | required | ✅ pass | 2404 ms | pass=141, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 205 ms | 42 tools |
| `doctor.hermes3d` | skipped | ✅ pass | 0 ms | skipped |
| `events.directory_present` | required | ✅ pass | 10 ms | outbox/handled/failed present |
| `tasks.directory_present` | required | ✅ pass | 4 ms | pending/claimed/blocked/done present |
| `trigger.doctor_passes` | required | ✅ pass | 62 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 140 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 58 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 304 ms | 14/14 checks; 15 ledger, 11 events |
...
| `lmstudio.health` | warn | ⚠️ warn | 10 ms | LM Studio offline: ECONNREFUSED |
| `ollama.health` | warn | ⚠️ warn | 1 ms | Ollama offline: ECONNREFUSED |
| `secret.scan` | required | ✅ pass | 39 ms | fallback: 0 finding(s) |
...
| `sbom.cyclonedx_generated` | required | ✅ pass | 34 ms | 131 components @ /home/runner/work/HermesProof/HermesProof/PROOF/sbom.json |
| `licenses.scan` | required | ✅ pass | 955 ms | 93 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 166 ms | 3/3 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 3 ms | 4 workflow(s), 17 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 1452 ms | 0 critical/serious violations across 22 passing rule(s) (1 non-blocking warning(s)) |
| `perf.budgets_pass` | required | ✅ pass | 3127 ms | hermes_doctor_cold_start=0.4ms<300ms? Y; lock_acquire=6.4ms<50ms? Y; heartbeat=2.1ms<20ms? Y |
| `docs.reflects_changes` | warn | ✅ pass | 7 ms | no version bump or ADR change in range; gate is inert |
| `release.checksums_present` | warn | ✅ pass | 1 ms | no release artifacts in dist,release; gate dormant |
| `quality.coderabbit_reviewed` | skipped | ✅ pass | 3 ms | no PR context (owner/repo/pr); gate inert |
