...
- **Run id**: `truth_2026-07-02T13-38-57-038Z`
- **Timestamp (UTC)**: 2026-07-02T13:38:57.038Z
- **Duration**: 47.12s
...
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 20 ms | 76 files hashed |
| `deps.parity` | required | ✅ pass | 1 ms | all 3 deps installed |
| `tests.unit` | required | ✅ pass | 31641 ms | pass=304, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 389 ms | 68 tools |
| `doctor.hermes3d` | skipped | ✅ pass | 0 ms | skipped |
| `events.directory_present` | required | ✅ pass | 26 ms | outbox/handled/failed present |
| `tasks.directory_present` | required | ✅ pass | 22 ms | pending/claimed/blocked/done present |
| `trigger.doctor_passes` | required | ✅ pass | 335 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 533 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 544 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 1042 ms | 14/14 checks; 15 ledger, 11 events |
...
| `security.mcp_scan_pass` | required | ✅ pass | 55 ms | 0 suspicious patterns across 16 signatures |
...
| `backend.api_config_presence` | warn | ✅ pass | 260 ms | backend API inventory: envs=10, gh=true, glab=false |
| `gitlab.auth_probe` | warn | ⚠️ warn | 1 ms | GitLab token not configured (set GITLAB_TOKEN or GLAB_TOKEN) |
| `secret.scan` | required | ✅ pass | 113 ms | fallback: 0 finding(s) |
| `secrets.rotation_evidence_present` | warn | ✅ pass | 0 ms | env mtime 2026-06-20T17:43:43.876Z (age 11.8d <= max 90d) |
| `sbom.cyclonedx_generated` | required | ✅ pass | 70 ms | 131 components @ G:/Github/hermes3d-mcp-lock-orchestrator/PROOF/sbom.json |
| `licenses.scan` | required | ✅ pass | 7568 ms | 93 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 626 ms | 3/3 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 4 ms | 4 workflow(s), 17 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 2563 ms | 0 critical/serious violations across 22 passing rule(s) (1 non-blocking warning(s)) |
...
| `docs.reflects_changes` | warn | ✅ pass | 195 ms | no version bump or ADR change in range; gate is inert |
