...
- **Run id**: `truth_2026-08-09T19-08-44-671Z`
- **Timestamp (UTC)**: 2026-08-09T19:08:44.671Z
- **Duration**: 100.83s
...
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 41 ms | 184 files hashed |
| `deps.parity` | required | ✅ pass | 2 ms | all 7 deps installed |
| `tests.unit` | required | ✅ pass | 86666 ms | pass=495, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 490 ms | 121 tools |
| `doctor.hermes3d` | required | ✅ pass | 5 ms | ok=true, 0 finding(s) |
| `events.directory_present` | required | ✅ pass | 100 ms | outbox/handled/failed present |
| `tasks.directory_present` | required | ✅ pass | 22 ms | pending/claimed/blocked/done present |
| `trigger.doctor_passes` | required | ✅ pass | 269 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 421 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 545 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 1218 ms | 14/14 checks; 15 ledger, 11 events |
| `workspace.integrity` | required | ✅ pass | 200 ms | probes=0, install_mods=0, expected_mods=0, unexpected_mods=0, unexpected_untracked=0 |
| `clients.config_presence` | required | ✅ pass | 2 ms | all 4 present |
| `clients.claude_code_live` | required | ✅ pass | 5140 ms | Connected |
| `server.tool_description_hygiene` | required | ✅ pass | 7 ms | 0 suspicious patterns |
| `security.mcp_scan_pass` | required | ✅ pass | 112 ms | 0 suspicious patterns across 16 signatures |
...
| `lmstudio.health` | warn | ✅ pass | 1030 ms | LM Studio reachable via local fallback (200) |
| `ollama.health` | warn | ✅ pass | 2 ms | Ollama reachable (200) |
| `backend.api_config_presence` | warn | ✅ pass | 227 ms | backend API inventory: envs=8, gh=true, glab=false |
| `gitlab.auth_probe` | warn | ✅ pass | 292 ms | GitLab authenticated via gitlab_env_file:GITLAB_TOKEN |
| `secret.scan` | required | ✅ pass | 148 ms | fallback: 0 finding(s) |
| `secrets.rotation_evidence_present` | warn | ✅ pass | 1 ms | env mtime 2026-08-08T19:54:02.512Z (age 1.0d <= max 90d) |
| `sbom.cyclonedx_generated` | required | ✅ pass | 37 ms | 96 components @ C:/Users/Admin/Documents/Codex/2026-08-08/g-github-hermes3d-mcp-lock-orchestrator-2/work/hermesproof-release/PROOF/sbom.json |
| `licenses.scan` | required | ✅ pass | 1398 ms | 99 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 1262 ms | 7/7 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 0 ms | 0 workflow(s), 0 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 996 ms | 0 critical/serious violations across 21 passing rule(s) (1 non-blocking warning(s)) |
...
| `docs.reflects_changes` | warn | ✅ pass | 94 ms | no version bump or ADR change in range; gate is inert |
| `release.checksums_present` | warn | ✅ pass | 11 ms | 1 artifact(s) have verified sha256+Ed25519 signatures |
...
| `harness_attribution.holdout_isolation_at_queue` | required | ✅ pass | 14 ms | cases=ok\|4/4 (optimizer_on_holdout=FAIL, agent_on_holdout=PASS, optimizer_on_optimization=PASS, optimizer_on_mixed=FAIL) \| index rows=2 range-match=1 |
