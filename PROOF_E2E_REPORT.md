...
- **Run id**: `truth_2026-05-03T20-11-07-163Z`
- **Timestamp (UTC)**: 2026-05-03T20:11:07.163Z
- **Duration**: 8.35s
...
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 13 ms | 73 files hashed |
| `deps.parity` | required | ✅ pass | 2 ms | all 3 deps installed |
| `tests.unit` | required | ✅ pass | 1951 ms | pass=141, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 183 ms | 42 tools |
...
| `trigger.doctor_passes` | required | ✅ pass | 59 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 151 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 54 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 283 ms | 14/14 checks; 15 ledger, 11 events |
...
| `docs.master_prompt_deliverables_present` | required | ✅ pass | 1 ms | 10/10 deliverables present |
...
| `lmstudio.health` | warn | ⚠️ warn | 9 ms | LM Studio offline: ECONNREFUSED |
| `ollama.health` | warn | ⚠️ warn | 1 ms | Ollama offline: ECONNREFUSED |
| `secret.scan` | required | ✅ pass | 36 ms | fallback: 0 finding(s) |
...
| `sbom.cyclonedx_generated` | required | ✅ pass | 35 ms | 131 components @ /home/runner/work/HermesProof/HermesProof/PROOF/sbom.json |
| `licenses.scan` | required | ✅ pass | 806 ms | 93 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 156 ms | 3/3 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 2 ms | 4 workflow(s), 17 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 1238 ms | 0 critical/serious violations across 22 passing rule(s) (1 non-blocking warning(s)) |
| `perf.budgets_pass` | required | ✅ pass | 3324 ms | hermes_doctor_cold_start=0.4ms<300ms? Y; lock_acquire=5.7ms<50ms? Y; heartbeat=1.9ms<20ms? Y |
| `docs.reflects_changes` | warn | ✅ pass | 8 ms | no version bump or ADR change in range; gate is inert |
