...
- **Run id**: `truth_2026-08-12T21-12-34-715Z`
- **Timestamp (UTC)**: 2026-08-12T21:12:34.715Z
- **Duration**: 48.93s
...
| `tests.unit` | required | ✅ pass | 31386 ms | pass=497, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 374 ms | 121 tools |
...
| `trigger.doctor_passes` | required | ✅ pass | 302 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 420 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 519 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 775 ms | 14/14 checks; 15 ledger, 11 events |
| `workspace.integrity` | required | ✅ pass | 207 ms | probes=0, install_mods=0, expected_mods=0, unexpected_mods=0, unexpected_untracked=0 |
| `clients.config_presence` | required | ✅ pass | 2 ms | all 4 present |
| `clients.claude_code_live` | required | ✅ pass | 4934 ms | 2/2 connected |
| `server.tool_description_hygiene` | required | ✅ pass | 10 ms | 0 suspicious patterns |
| `security.mcp_scan_pass` | required | ✅ pass | 116 ms | 0 suspicious patterns across 16 signatures |
| `evidence.hash_chain_valid` | required | ✅ pass | 34 ms | positive=true, negative_detected_at_idx_1=true |
| `docs.master_prompt_deliverables_present` | required | ✅ pass | 5 ms | 10/10 deliverables present |
...
| `lmstudio.health` | warn | ⚠️ warn | 5019 ms | LM Studio offline: ECONNREFUSED |
| `ollama.health` | warn | ✅ pass | 4 ms | Ollama reachable (200) |
| `backend.api_config_presence` | warn | ✅ pass | 226 ms | backend API inventory: envs=8, gh=true, glab=false |
| `gitlab.auth_probe` | warn | ✅ pass | 302 ms | GitLab authenticated via gitlab_env_file:GITLAB_TOKEN |
| `secret.scan` | required | ✅ pass | 164 ms | fallback: 0 finding(s) |
| `secrets.rotation_evidence_present` | warn | ✅ pass | 0 ms | env mtime 2026-08-08T19:54:02.512Z (age 4.1d <= max 90d) |
| `sbom.cyclonedx_generated` | required | ✅ pass | 44 ms | 96 components @ C:/Users/Admin/Documents/Codex/2026-08-08/g-github-hermes3d-mcp-lock-orchestrator-2/work/hermesproof-release/PROOF/sbom.json |
| `licenses.scan` | required | ✅ pass | 1527 ms | 99 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 1162 ms | 7/7 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 1 ms | 0 workflow(s), 0 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 1053 ms | 0 critical/serious violations across 21 passing rule(s) (1 non-blocking warning(s)) |
...
| `docs.reflects_changes` | warn | ✅ pass | 170 ms | no version bump or ADR change in range; gate is inert |
| `release.checksums_present` | warn | ✅ pass | 15 ms | 1 artifact(s) have verified sha256+Ed25519 signatures |
| `quality.coderabbit_reviewed` | skipped | ✅ pass | 33 ms | no PR context (owner/repo/pr); gate inert |
...
| `harness_attribution.holdout_isolation_at_queue` | required | ✅ pass | 5 ms | cases=ok\|4/4 (optimizer_on_holdout=FAIL, agent_on_holdout=PASS, optimizer_on_optimization=PASS, optimizer_on_mixed=FAIL) \| index rows=2 range-match=1 |
