...
- **Run id**: `truth_2026-07-02T16-53-21-931Z`
- **Timestamp (UTC)**: 2026-07-02T16:53:21.931Z
- **Duration**: 46.40s
...
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 30 ms | 77 files hashed |
| `deps.parity` | required | ✅ pass | 2 ms | all 3 deps installed |
| `tests.unit` | required | ✅ pass | 31633 ms | pass=309, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 504 ms | 82 tools |
| `doctor.hermes3d` | skipped | ✅ pass | 0 ms | skipped |
| `events.directory_present` | required | ✅ pass | 51 ms | outbox/handled/failed present |
| `tasks.directory_present` | required | ✅ pass | 26 ms | pending/claimed/blocked/done present |
| `trigger.doctor_passes` | required | ✅ pass | 379 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 488 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 507 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 1009 ms | 14/14 checks; 15 ledger, 11 events |
...
| `security.mcp_scan_pass` | required | ✅ pass | 85 ms | 0 suspicious patterns across 16 signatures |
...
| `backend.api_config_presence` | warn | ✅ pass | 372 ms | backend API inventory: envs=10, gh=true, glab=false |
| `gitlab.auth_probe` | warn | ✅ pass | 305 ms | GitLab authenticated via gitlab_env_file:GITLAB_TOKEN |
| `secret.scan` | required | ✅ pass | 123 ms | fallback: 0 finding(s) |
| `secrets.rotation_evidence_present` | warn | ✅ pass | 0 ms | env mtime 2026-06-20T17:43:43.876Z (age 12.0d <= max 90d) |
| `sbom.cyclonedx_generated` | required | ✅ pass | 63 ms | 131 components @ G:/Github/hermes3d-mcp-lock-orchestrator/PROOF/sbom.json |
| `licenses.scan` | required | ✅ pass | 7724 ms | 93 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 631 ms | 3/3 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 3 ms | 4 workflow(s), 17 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 2262 ms | 0 critical/serious violations across 22 passing rule(s) (1 non-blocking warning(s)) |
...
| `docs.reflects_changes` | warn | ✅ pass | 118 ms | no version bump or ADR change in range; gate is inert |
