...
- **Run id**: `truth_2026-05-03T18-56-43-268Z`
- **Timestamp (UTC)**: 2026-05-03T18:56:43.268Z
- **Duration**: 11.23s
...
| `tests.unit` | required | ✅ pass | 2563 ms | pass=140, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 251 ms | 42 tools |
...
| `trigger.doctor_passes` | required | ✅ pass | 74 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 178 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 65 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 384 ms | 14/14 checks; 15 ledger, 11 events |
...
| `security.mcp_scan_pass` | required | ✅ pass | 10 ms | 0 suspicious patterns across 16 signatures |
...
| `lmstudio.health` | warn | ⚠️ warn | 12 ms | LM Studio offline: ECONNREFUSED |
| `ollama.health` | warn | ⚠️ warn | 2 ms | Ollama offline: ECONNREFUSED |
| `secret.scan` | required | ✅ pass | 56 ms | fallback: 0 finding(s) |
...
| `licenses.scan` | required | ✅ pass | 1071 ms | 93 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 180 ms | 3/3 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 2 ms | 4 workflow(s), 17 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 1705 ms | 0 critical/serious violations across 22 passing rule(s) (1 non-blocking warning(s)) |
| `perf.budgets_pass` | required | ✅ pass | 4546 ms | hermes_doctor_cold_start=0.6ms<300ms? Y; lock_acquire=8.4ms<50ms? Y; heartbeat=2.9ms<20ms? Y |
| `docs.reflects_changes` | warn | ✅ pass | 12 ms | no version bump or ADR change in range; gate is inert |
| `release.checksums_present` | warn | ✅ pass | 0 ms | no release artifacts in dist,release; gate dormant |
| `quality.coderabbit_reviewed` | skipped | ✅ pass | 5 ms | no PR context (owner/repo/pr); gate inert |
