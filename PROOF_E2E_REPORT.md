...
- **Run id**: `truth_2026-05-03T19-49-52-925Z`
- **Timestamp (UTC)**: 2026-05-03T19:49:52.925Z
- **Duration**: 11.42s
...
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 18 ms | 72 files hashed |
| `deps.parity` | required | ✅ pass | 2 ms | all 3 deps installed |
| `tests.unit` | required | ✅ pass | 2736 ms | pass=141, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 248 ms | 42 tools |
...
| `trigger.doctor_passes` | required | ✅ pass | 76 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 176 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 68 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 390 ms | 14/14 checks; 15 ledger, 11 events |
...
| `licenses.scan` | required | ✅ pass | 1077 ms | 93 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 340 ms | 3/3 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 3 ms | 4 workflow(s), 17 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 1714 ms | 0 critical/serious violations across 22 passing rule(s) (1 non-blocking warning(s)) |
| `perf.budgets_pass` | required | ✅ pass | 4389 ms | hermes_doctor_cold_start=0.6ms<300ms? Y; lock_acquire=7.8ms<50ms? Y; heartbeat=2.5ms<20ms? Y |
