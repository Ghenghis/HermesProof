...
- **Run id**: `truth_2026-05-03T18-14-46-985Z`
- **Timestamp (UTC)**: 2026-05-03T18:14:46.985Z
- **Duration**: 10.64s
...
| --- | --- | --- | --- | --- |
| `source.integrity_manifest` | required | ✅ pass | 16 ms | 67 files hashed |
| `deps.parity` | required | ✅ pass | 2 ms | all 3 deps installed |
| `tests.unit` | required | ✅ pass | 2532 ms | pass=140, fail=0, exit=0 |
| `server.stdio_handshake` | required | ✅ pass | 236 ms | 42 tools |
...
| `trigger.doctor_passes` | required | ✅ pass | 77 ms | trigger doctor ok |
| `queue.doctor_passes` | required | ✅ pass | 182 ms | queue doctor ok |
| `wizard.dry_run_passes` | required | ✅ pass | 67 ms | wizard dry-run ok |
| `e2e.multi_agent_flow` | required | ✅ pass | 378 ms | 14/14 checks; 15 ledger, 11 events |
...
| `secret.scan` | required | ✅ pass | 40 ms | fallback: 0 finding(s) |
...
| `sbom.cyclonedx_generated` | required | ✅ pass | 47 ms | 131 components @ /home/runner/work/HermesProof/HermesProof/PROOF/sbom.json |
| `licenses.scan` | required | ✅ pass | 968 ms | 93 packages scanned; unknown=0, review=1 |
| `dependency.fresh` | warn | ✅ pass | 204 ms | 3/3 direct deps within 12mo |
| `security.workflow_actions_sha_pinned` | required | ✅ pass | 3 ms | 4 workflow(s), 17 uses-ref(s), all SHA-pinned |
| `accessibility.wcag_aa_pass` | required | ✅ pass | 1572 ms | 0 critical/serious violations across 22 passing rule(s) (1 non-blocking warning(s)) |
| `perf.budgets_pass` | required | ✅ pass | 4247 ms | hermes_doctor_cold_start=0.5ms<300ms? Y; lock_acquire=8.3ms<50ms? Y; heartbeat=2.4ms<20ms? Y |
| `docs.reflects_changes` | warn | ✅ pass | 11 ms | no version bump or ADR change in range; gate is inert |
